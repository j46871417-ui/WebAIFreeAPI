...
WebAIFreeAPI спроектирован как **персональный локальный мост разработчика** (Personal Developer Bridge & Local Gateway). Он позволяет легитимному владельцу персональных аккаунтов использовать возможности нейросетей (DeepSeek, Qwen, ChatGPT) непосредственно в своём привычном рабочем окружении (IDE, CLI, локальные редакторы и терминал).

### Ключевые архитектурные решения и безопасность:
- 🔒 **Полная локальность и приватность**: всё взаимодействие происходит строго на локальном компьютере (`localhost:4317` / `127.0.0.1`). В проекте полностью отсутствуют внешние телеметрические серверы, облачные сборщики или сторонние прокси-шлюзы.
- 👤 **Персональная автоматизация браузерной сессии (Personal Automation)**: вы авторизуетесь самостоятельно в своём официальном аккаунте через изолированный браузерный профиль Chromium под управлением Playwright/Patchright. Сессия, токены и cookies хранятся в вашей домашней папке (`~/.deepseek-cli/`, `~/.qwen-cli/`, `~/.chatgpt-cli/`) под защитой системных прав доступа ОС.
- 🔌 **Стандартный протокол OpenAI API (`/v1/chat/completions`)**: приложение выступает высокопроизводительным адаптером протоколов, преобразующим веб-ответы в стандартные потоки SSE (Server-Sent Events) со структурированным вызовом функций (Tool Calling) для любых сред разработки.
- 🛡️ **Песочница рабочего пространства (Workspace Sandboxing)**: автономный кодовый агент выполняет операции строго в границах выбранного каталога проекта (`workspaceRoot`). Выход за пределы рабочей папки блокируется, а запуск терминальных утилит контролируется настраиваемым каталогом безопасности (`COMMAND_CATALOG`).
- 🤝 **Ответственное использование**: приложение предназначено для индивидуального использования разработчиками, уважает пользовательские лимиты веб-сервисов и предотвращает паразитные повторные запросы за счёт умного локального кэширования контекста.

---

## 📂 Команды для работы с файлами, проектом и терминалом

В окне приложения, в строке ввода и в консольном CLI поддерживаются явные команды для прямого взаимодействия с проектом:

### 📄 Работа с отдельными файлами: `/file` и `/read`
- **Анализ файла проекта**:
  ```
  /file src/window-app/server.mjs
  ```
- **Задача или вопрос по конкретному файлу**:
  ```
  /file "src/code-agent/task-input.mjs" как устроен парсинг аргументов?
  ```
  *(В строке ввода доступна быстрая кнопка **`📄 /file`**, вставляющая шаблон команды).*

### 📁 Обзор структуры и каталогов: `/folder` и `/dir`
- **Сканирование папки проекта**:
  ```
  /folder src/code-agent
  ```
- **Анализ файлов директории под задачу**:
  ```
  /folder test найди все тесты для агентских команд
  ```
  *(В строке ввода доступна быстрая кнопка **`📁 /folder`**).*

### 💻 Выполнение команд в терминале: `/terminal`, `/term`, `/cmd`, `/sh`, `/run`
- **Запуск команды в рабочей директории**:
  ```
  /terminal npm test
  /cmd git status
  /term node scripts/check-ci-invariants.mjs
  ```
- Команда исполняется встроенными инструментами агента (`run_command` / `run_shell`) в окружении выбранного проекта. Агент перехватывает вывод `stdout` / `stderr`, анализирует ошибки и даёт исчерпывающий ответ разработчику.
  *(В строке ввода доступна быстрая кнопка **`💻 /terminal`**).*

### 🛠 Полноценные задачи по коду: `/code` и `/skill`
- `/code <задача>` — запускает автономный цикл агента (поиск по коду, чтение файлов, внесение правок, проверка тестами).
- `/skill <id> <задача>` — применяет специализированные профили (например, `/skill bug-fix`, `/skill code-review`).
